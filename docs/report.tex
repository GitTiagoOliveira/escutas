...
\newglossaryentry{caching}{
    name = Caching,
    description = {Técnica que consiste em armazenar, de forma temporária, cópias de dados ou ficheiros, para estes sejam mais rapidamente acedidos}
}
...
\begin{itemize}
    ...
    \item \emph{News Feed}: primeira vista a ser carregada após se efectuar a autenticação. Mostra as últimas notícias acerca do mundo do escutismo, sob a forma de lista, contendo o seu título, corpo e imagem. Quando o utilizador prime alguma das notícias presentes na lista, é reencaminhado para uma nova vista, onde a notícia é exibida em maior detalhe. De seguida, é possível verificar estas vistas, através das figuras 12 e 13:
    
    \bigskip
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.34\textwidth]{view-news-feed-1.png}
        \caption{Vista News Feed}
    \end{figure}
    
    \bigskip
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.34\textwidth]{view-news-feed-2.png}
        \caption{Vista News Feed (continuação)}
    \end{figure}
    
    \item \emph{Map}: vista encarregue de apresentar, através de um mapa interactivo, os grupos de escutismo que se encontram nas proximidades do utilizador da aplicação. Para tal, recorre à \gls{api} do \gls{maps} e a mecanismos de localização do sistema operativo \gls{android}. Por sua vez, o referido sistema operativo utiliza o sensor \gls{gps} do dispositivo. Seguindo as boas práticas de desenvolvimento \gls{android}, as permissões de localização são meramente requeridas no momento em que se tornam indispensáveis. Perceba-se a vista na figura 14 que se segue:
    ...
    \item \emph{Calendar}: vista que oferece, ao utilizador, a possibilidade de ver os eventos existentes na comunidade ou criar um novo evento. Ao optar por criar um novo evento, é apresentada uma vista que permite ao utilizador adicionar o evento ao calendário, se este preencher correctamente os campos apresentados. Poderá escolher as datas através dos calendários, as horas através dos relógios ou até uma imagem para servir como capa do evento. Os eventos são expostos da mesma forma que as notícias, com o acrescento do dia e mês do evento, como se pode constatar nas figuras 15 e 16:
